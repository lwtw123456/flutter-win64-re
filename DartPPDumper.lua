...
local resolvedStringCids = {
    one = nil,
    two = nil
}

local STRING_CALIBRATE_MAX_LEN = 4096
local STRING_CALIBRATE_LIMIT   = 0x40000
...
local function roundUp16(v)
    return ((v + 0x0F) // 0x10) * 0x10
end

local function payloadFitsHeader(h, byteCount)
    if h == nil or byteCount < 0 then return false, false end

    local minSize = 0x10 + byteCount
    if h.size ~= nil and minSize > h.size then
        return false, false
    end

    local exact = h.size ~= nil and h.size == roundUp16(minSize)
    return true, exact
end

local function validateUtf8Text(bytes, len)
    if bytes == nil or len <= 0 or #bytes < len then return false, nil end
    ...
    while i <= len do
        local b1 = bytes[i]
        if b1 == nil or b1 < 0 or b1 >= 0x100 then return false, nil end
        ...
        elseif b1 >= 0xC2 and b1 <= 0xDF then
            if i + 1 > len then return false, nil end
            local b2 = bytes[i + 1]
            if b2 < 0x80 or b2 > 0xBF then return false, nil end
            cp = ((b1 & 0x1F) << 6) | (b2 & 0x3F)
            used = 2
        elseif b1 >= 0xE0 and b1 <= 0xEF then
            if i + 2 > len then return false, nil end
            local b2, b3 = bytes[i + 1], bytes[i + 2]
            if b2 < 0x80 or b2 > 0xBF or b3 < 0x80 or b3 > 0xBF then
                return false, nil
            end
            if b1 == 0xE0 and b2 < 0xA0 then return false, nil end
            if b1 == 0xED and b2 > 0x9F then return false, nil end
            cp = ((b1 & 0x0F) << 12) | ((b2 & 0x3F) << 6) | (b3 & 0x3F)
            used = 3
        elseif b1 >= 0xF0 and b1 <= 0xF4 then
            if i + 3 > len then return false, nil end
            local b2, b3, b4 = bytes[i + 1], bytes[i + 2], bytes[i + 3]
            if b2 < 0x80 or b2 > 0xBF or
               b3 < 0x80 or b3 > 0xBF or
               b4 < 0x80 or b4 > 0xBF then
                return false, nil
            end
            if b1 == 0xF0 and b2 < 0x90 then return false, nil end
            if b1 == 0xF4 and b2 > 0x8F then return false, nil end
        ...
        if cp == 0 then return false, nil end
        if cp == 0x09 or cp == 0x0A or cp == 0x0D or cp >= 0x20 then
            printable = printable + 1
        end
        if cp <= 0x7F then ascii = ascii + 1 end

        codepoints = codepoints + 1
        i = i + used
    end

    if codepoints == 0 then return false, nil end
    if printable * 100 < codepoints * 75 then return false, nil end
...
local function validateUtf16leText(bytes, codeUnits)
    if bytes == nil or codeUnits <= 0 or #bytes < codeUnits * 2 then
        return false, nil
    end
    ...
    local zeroHigh = 0
    local nonLatin1 = 0
    local surrogatePairs = 0

    while i < codeUnits do
        local p = i * 2 + 1
        local lo = bytes[p]
        local hi = bytes[p + 1]
        if lo == nil or hi == nil or lo < 0 or lo >= 0x100 or hi < 0 or hi >= 0x100 then
            return false, nil
        end

        if hi == 0 then zeroHigh = zeroHigh + 1 end
        ...
        if u >= 0xD800 and u <= 0xDBFF then
            if i >= codeUnits then return false, nil end
            local p2 = i * 2 + 1
            local u2 = (bytes[p2] or 0) | ((bytes[p2 + 1] or 0) << 8)
            if u2 < 0xDC00 or u2 > 0xDFFF then return false, nil end
            cp = 0x10000 + ((u - 0xD800) << 10) + (u2 - 0xDC00)
            surrogatePairs = surrogatePairs + 1
            i = i + 1
        elseif u >= 0xDC00 and u <= 0xDFFF then
            return false, nil
        end

        if cp > 0x10FFFF or cp == 0 then return false, nil end
        if cp == 0x09 or cp == 0x0A or cp == 0x0D or cp >= 0x20 then
            printable = printable + 1
        end
        if cp > 0xFF then nonLatin1 = nonLatin1 + 1 end
        codepoints = codepoints + 1
    end
...
local function probeOneByteString(rawAddr, h)
    local len = readTaggedStringLength(rawAddr)
    if len == nil or len <= 0 or len > STRING_CALIBRATE_MAX_LEN then
        return false, 0, false
    end

    local fits, exact = payloadFitsHeader(h, len)
    if not fits then return false, 0, false end

    local bytes = readExactBytes(rawAddr + 0x10, len)
    local ok, stats = validateUtf8Text(bytes, len)
    if not ok then return false, 0, false end

    local score = 2
    if exact then score = score + 2 end
    if stats.ascii == stats.codepoints then
        score = score + 2
    else
        score = score + 1
    end
...
local function probeTwoByteString(rawAddr, h)
    local len = readTaggedStringLength(rawAddr)
    if len == nil or len <= 0 or len > STRING_CALIBRATE_MAX_LEN then
        return false, 0, false
    end

    local byteCount = len * 2
    local fits, exact = payloadFitsHeader(h, byteCount)
    if not fits then return false, 0, false end

    local bytes = readExactBytes(rawAddr + 0x10, byteCount)
    local ok, stats = validateUtf16leText(bytes, len)
    if not ok then return false, 0, false end

    local score = 2
    if exact then score = score + 2 end

    if stats.zeroHigh * 100 >= len * 40 then score = score + 3 end
    if stats.nonLatin1 > 0 then score = score + 3 end
    if stats.surrogatePairs > 0 then score = score + 4 end

    return true, score, exact
end

local function addStringCidEvidence(kind, cid, score, decisive)
    local bucket = dartStringCids[kind]
    local e = bucket[cid]
    if e == nil then
        e = { hits = 0, score = 0, decisive = 0 }
        bucket[cid] = e
    end
    e.hits = e.hits + 1
    e.score = e.score + (score or 0)
    if decisive then e.decisive = e.decisive + 1 end
end

local function candidateMetric(kind, cid)
    local e = dartStringCids[kind][cid]
    if e == nil then return 0, 0, 0 end
    return e.score + e.decisive * 8, e.hits, e.decisive
end

local function chooseResolvedCid(kind, excludedCid)
    local other = (kind == "one") and "two" or "one"
    local bestCid, bestAffinity, bestMetric = nil, -0x7FFFFFFF, 0

    for cid, e in pairs(dartStringCids[kind]) do
        if cid ~= excludedCid then
            local ownMetric = e.score + e.decisive * 8
            local otherMetric = candidateMetric(other, cid)
            local affinity = ownMetric - otherMetric

            local enoughEvidence = (e.hits >= 2) or (e.decisive >= 1 and ownMetric >= 6)
            if enoughEvidence and affinity >= 3 then
                if bestCid == nil or affinity > bestAffinity or
                   (affinity == bestAffinity and ownMetric > bestMetric) then
                    bestCid = cid
                    bestAffinity = affinity
                    bestMetric = ownMetric
                end
            end
        end
    end

    return bestCid, bestAffinity, bestMetric
end

local function resolveStringCidCandidates()
    local oneCid, oneAffinity = chooseResolvedCid("one", nil)
    local twoCid, twoAffinity = chooseResolvedCid("two", oneCid)

    if twoCid == nil then
        local tCid, tAffinity = chooseResolvedCid("two", nil)
        if tCid ~= nil then
            local oCid, oAffinity = chooseResolvedCid("one", tCid)
            if oCid ~= nil then
                oneCid, oneAffinity = oCid, oAffinity
                twoCid, twoAffinity = tCid, tAffinity
            end
        end
    end

    resolvedStringCids.one = oneCid
    resolvedStringCids.two = twoCid
    return oneCid, twoCid, oneAffinity, twoAffinity
end
...
local function calibrateStringCids(startAddr, entryBitsAddr, entryCount)
    dartStringCids.one = {}
    dartStringCids.two = {}
    resolvedStringCids.one = nil
    resolvedStringCids.two = nil

    local seenRaw = {}
    local limit = math.min(entryCount, STRING_CALIBRATE_LIMIT)
    ...
    for i = 0, limit - 1 do
        ...
        if typeId == POOL_TYPE.TAGGED_OBJECT then
            ...
            if tagged ~= nil and tagged >= 0x10000 and (tagged & 1) ~= 0 then
                local rawAddr = tagged - 1
                local rawKey = string.format("%016X", rawAddr)
                if not seenRaw[rawKey] then
                    seenRaw[rawKey] = true

                    local h = parseHeader(rawAddr)
                    if h ~= nil then
                        local oneOk, oneScore, oneExact = probeOneByteString(rawAddr, h)
                        local twoOk, twoScore, twoExact = probeTwoByteString(rawAddr, h)

                        if oneOk and not twoOk then
                            addStringCidEvidence("one", h.cid, oneScore + 2, true)
                        elseif twoOk and not oneOk then
                            addStringCidEvidence("two", h.cid, twoScore + 2, true)
                        elseif oneOk and twoOk then
                            if oneExact and not twoExact then
                                addStringCidEvidence("one", h.cid, oneScore + 2, true)
                                addStringCidEvidence("two", h.cid, math.max(1, twoScore - 2), false)
                            elseif twoExact and not oneExact then
                                addStringCidEvidence("two", h.cid, twoScore + 2, true)
                                addStringCidEvidence("one", h.cid, math.max(1, oneScore - 2), false)
                            else
                                addStringCidEvidence("one", h.cid, oneScore, false)
                                addStringCidEvidence("two", h.cid, twoScore, false)
                            end
                        end
                    end
                end
        ...
        if (i + 1) % 8192 == 0 then
            local oneCid, twoCid, oneAffinity, twoAffinity = resolveStringCidCandidates()
            if oneCid ~= nil and twoCid ~= nil and
               (oneAffinity or 0) >= 24 and (twoAffinity or 0) >= 24 then
                break
            end
            processMessages()
        end
    ...
    if oneCid ~= nil then
        local m, hits, decisive = candidateMetric("one", oneCid)
        print(string.format(
            "[String] OneByte CID=0x%X hits=%d decisive=%d metric=%d affinity=%d",
            oneCid, hits, decisive, m, oneAffinity or 0))
    else
        print("[String] OneByte CID 未确认")
    end

    if twoCid ~= nil then
        local m, hits, decisive = candidateMetric("two", twoCid)
        print(string.format(
            "[String] TwoByte CID=0x%X hits=%d decisive=%d metric=%d affinity=%d",
            twoCid, hits, decisive, m, twoAffinity or 0))
    else
        print("[String] TwoByte CID 未确认")
    end

    return oneCid ~= nil or twoCid ~= nil
end
...
local function readDartString(rawAddr)
    ...
    local kind
    if resolvedStringCids.one ~= nil and h.cid == resolvedStringCids.one then
        kind = "OneByte"
    elseif resolvedStringCids.two ~= nil and h.cid == resolvedStringCids.two then
        kind = "TwoByte"
    else
        return nil
    end
    ...
    local fits = payloadFitsHeader(h, byteCount)
    ...
    if kind == "TwoByte" then
        local ok16 = validateUtf16leText(bytes, len)
        if not ok16 then return nil end
...
local function isStringHeader(h)
    ...
    return (resolvedStringCids.one ~= nil and h.cid == resolvedStringCids.one) or
           (resolvedStringCids.two ~= nil and h.cid == resolvedStringCids.two)
end
...
local function dumpObjectPool(pp)
    ...
    if not calibrateStringCids(startAddr, entryBitsAddr, entryCount) then
        print("[String] 未确认任何 String CID；字符串将保守按普通 Object 输出")
    elseif resolvedStringCids.one == nil or resolvedStringCids.two == nil then
        print("[String] 仅确认一种 String CID；未确认的类型将保守按普通 Object 输出")
    end
